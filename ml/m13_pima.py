...
# 붓꽃 데이터 읽어 들이기
iris_data = pd.read_csv("./data/csv/pima-indians-diabetes.csv", encoding='utf-8',
                        names=['a', 'b', 'c', 'd', 'y'])
# print(iris_data)
# 붓꽃 데이터를 레이블과 입력 데이터로 분리하기
y = iris_data.loc[:, "y"] # .loc 레이블로 나누기(레이블은 실질적인 데이터가 아님)
x = iris_data.loc[:,["a", "b", "c", "d", "e", "f", "g", "h"]]
...
# 학습 전용과 테스트 전용 분리하기
x_train, x_test, y_train, y_test = train_test_split(x_array, y, test_size=0.2, train_size=0.8, shuffle=True)
